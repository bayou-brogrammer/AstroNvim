...
M.formatters = {} ---@type AtomicFormatter[]

---@param formatter AtomicFormatter
...
function M.formatexpr()
  if AtomicVim.has "conform.nvim" then return require("conform").formatexpr() end
  return vim.lsp.formatexpr { timeout_ms = 3000 }
end

---@param buf? number
---@return (AtomicFormatter|{active:boolean,resolved:string[]})[]
...
function M.info(buf)
  ...
  AtomicVim[enabled and "info" or "warn"](
    table.concat(lines, "\n"),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then AtomicVim.warn("No formatter available", { title = "LazyVim" }) end
end
...
function M.health()
  ...
  if has_plugin and not has_extra then
    AtomicVim.warn {
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in AtomicVim.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to enable the `none-ls` extra,",
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd("BufWritePre", {
    group = vim.api.nvim_create_augroup("AtomicFormat", {}),
